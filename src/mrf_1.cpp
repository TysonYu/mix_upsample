...
void MRF::MRFProcess()
{
            ...
            for(int i = 1; i < raw_image_.rows-1; i++)
            {
                for(int j = 1; j < raw_image_.cols-1; j++)
                {
                    if(M.at<float>(i,j) == max_depth_)
                    {
                        ...
                        if(neighbor_depth[0] < max_depth_ && gray_diference[0] < 20) flag[0] =1;
                        if(neighbor_depth[1] < max_depth_ && gray_diference[1] < 20) flag[1] =1;
                        if(neighbor_depth[2] < max_depth_ && gray_diference[2] < 20) flag[2] =1;
                        if(neighbor_depth[3] < max_depth_ && gray_diference[3] < 20) flag[3] =1;
                        for(int k = 0; k < 4; k++)
                        {
                            if(flag[k] == 1 && max_depth_ > neighbor_depth[k])
                            {
                                sum = sum + neighbor_depth[k] * (20 - gray_diference[k]);
                                cnt = cnt + (20 - gray_diference[k]);
                                count ++;
                            }
                        }
                        if(count > 0)
    ...
    for(int i = 0; i < TOTAL; i++)
    {
        ...
        result.at<float>(i/small_depth_image_2.size().width,i%small_depth_image_2.size().width) = x(i);
        result.at<float>(i/small_depth_image_2.size().width,i%small_depth_image_2.size().width + small_depth_image_2.size().width) = x2(i);
    }
    cv::Mat frame = result.clone();
    // cv::GaussianBlur(frame, result, cv::Size(0, 0), 3);
    // cv::addWeighted(frame, 2, result, -0.5, 0, result);
    // 锐化深度图边缘
    ...
    // sor.setInputCloud (result_cloud_rgb);
    // sor.setMeanK (1);
    // sor.setStddevMulThresh (1.5);
    // sor.filter (*result_cloud_rgb);
